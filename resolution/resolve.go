// Package main реализует движок резолюций для логического вывода
...
// resolvePair пытается создать резольвенту из двух клауз
func (e *ResolutionEngine) resolvePair(c1, c2 *Clause) *Clause {
	for _, l1 := range c1.Literals {
		for _, l2 := range c2.Literals {
			// Ищем пару L и ¬L
	...
	for {
		...
		for i := 0; i < len(currentPool); i++ {
			for j := i + 1; j < len(currentPool); j++ {
				...
				// для данной пары пытаемся найти резольвенту
				resolvent := e.resolvePair(c1, c2)
				...
				if resolvent != nil {
					...
					// если полученная резольвента новая, добавляем её в список активный клауз
					if !isDuplicate {
						...
						// если нашли противоречение - завершаем программу
						if isContradiction {
...
func test() {
	...
	r1 := []string{
		"¬Викинг(x) ∨ Храбрый(x)",
		"¬Храбрый(x) ∨ ПопадаетВВальгаллу(x)",
		"Викинг(Рагнар)",
		"¬ПопадаетВВальгаллу(Рагнар)",
	}

	complexAncestry := []string{
	...
	bigTestNetwork := []string{
		// ==========================================
		// 1. Правила (Аксиомы)
		// ==========================================

		// Аксиома 1: Прямое соединение создает путь
		"¬Соединение(x, y) ∨ Путь(x, y)",

		// Аксиома 2: Транзитивность пути (самая "тяжелая" часть для вычислений)
		// Если есть путь от x к y, и от y к z, то есть путь от x к z.
		"¬Путь(x, y) ∨ ¬Путь(y, z) ∨ Путь(x, z)",

		// Аксиома 3: Распространение вируса
		// Если узел x заражен и есть путь к y, то y тоже заражается.
		"¬Заражен(x) ∨ ¬Путь(x, y) ∨ Заражен(y)",

		// ==========================================
		// 2. Топология сети (Факты)
		// ==========================================

		// Внешний периметр
		"Соединение(Интернет, РоутерВнешний)",
		"Соединение(РоутерВнешний, Файрвол)",
		"Соединение(Файрвол, Балансировщик)",

		// Уровень сервисов (Ветвление 1)
		"Соединение(Балансировщик, ВебСервер1)",
		"Соединение(Балансировщик, ВебСервер2)",
		"Соединение(Балансировщик, ВебСервер3)",
		"Соединение(Балансировщик, АПИШлюз)",

		// Уровень логики (Ветвление 2)
		"Соединение(ВебСервер1, ЛогСервер)",
		"Соединение(ВебСервер2, ЛогСервер)",
		"Соединение(ВебСервер3, ЛогСервер)",
		"Соединение(АПИШлюз, СервисАвторизации)",
		"Соединение(АПИШлюз, СервисПлатежей)",

		// Перекрестные связи (усложняют граф)
		"Соединение(СервисАвторизации, ЛогСервер)",
		"Соединение(СервисПлатежей, ЛогСервер)",

		// Уровень хранения данных (Глубокий уровень)
		"Соединение(СервисАвторизации, КешЮзеров)",
		"Соединение(КешЮзеров, БазаЮзеров)",
		"Соединение(СервисПлатежей, БазаТранзакций)",

		// Административный сегмент (Скрытый путь)
		"Соединение(Файрвол, ВПНШлюз)",
		"Соединение(ВПНШлюз, АдминПК)",
		"Соединение(АдминПК, БазаЮзеров)", // Уязвимость через админа
		"Соединение(АдминПК, БазаТранзакций)",

		// Самое защищенное место (Цель)
		"Соединение(БазаЮзеров, БэкапСервер)",
		"Соединение(БазаТранзакций, БэкапСервер)",
		"Соединение(БэкапСервер, ХолодноеХранилище)",

		// ==========================================
		// 3. Начальные условия
		// ==========================================

		// Факт: Вирус попал в Интернет
		"Заражен(Интернет)",

		// ==========================================
		// 4. Цель доказательства (Отрицание)
		// ==========================================

		// Мы хотим доказать: Заражен(ХолодноеХранилище)
		// Добавляем отрицание:
		"¬Заражен(ХолодноеХранилище)",
	}
	...
	// Запуск теста (как в оригинале — testFriends)
	engine := NewResolutionEngine()
	engine.ParseInput(r1)
